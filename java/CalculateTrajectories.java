...
public class CalculateTrajectories {
    ...
    private final TreeSet[] nodes;               // Array of connected components
    private final int MAX_NODES;
    private final int rows;
    private final int cols;
    private final Vector MSTvector;
    private final int MSTedges;
    private final HashSet<Edge> AllEdges;
    private final ArrayList<Integer[]> PathSequence;


    public CalculateTrajectories(int r, int c, Vector MST) {
        // Constructor
        ...
        for (int i = 0; i < 2 * rows; i++) {
            for (int j = 0; j < 2 * cols; j++) {
                if (A[i][j]) {
                    ...
                    if (i < 2 * rows - 1 && A[i + 1][j]) {
                        AddToAllEdges(i * 2 * cols + j, (i + 1) * 2 * cols + j, 1);
                    }
                    if (j > 0 && A[i][j - 1]) {
                        AddToAllEdges(i * 2 * cols + j, i * 2 * cols + j - 1, 1);
                    }
                    if (j < 2 * cols - 1 && A[i][j + 1]) {
                        AddToAllEdges(i * 2 * cols + j, i * 2 * cols + j + 1, 1);
                    }
                    ...
                    if (!connect4) {
                        ...
                        if (i < 2 * rows - 1 && j < 2 * cols - 1 && A[i + 1][j + 1]) {
                            AddToAllEdges(i * 2 * cols + j, (i + 1) * 2 * cols + j + 1, 1);
                        }
                        if (i > 2 * rows - 1 && j > 0 && A[i + 1][j - 1]) {
                            AddToAllEdges(i * 2 * cols + j, (i + 1) * 2 * cols + j - 1, 1);
                        }
                        if (i > 0 && j < 2 * cols - 1 && A[i - 1][j + 1]) {
                            AddToAllEdges(i * 2 * cols + j, (i - 1) * 2 * cols + j + 1, 1);
                        }
                    }
                }
    ...
    public void CalculatePathsSequence(int StartingNode) {
        ...
        do {
            ...
            nodes[currentNode].remove(prevNode);
            nodes[prevNode].remove(currentNode);
