...
public class Kruskal {
    private final HashSet[] nodes;               // Array of connected components
    private final TreeSet allEdges;              // Priority queue of Edge objects
    private final Vector allNewEdges;            // Edges in Minimal-Spanning Tree
    private final int MAX_NODES;

    public Kruskal(int maxN) {
        // Constructor
        this.MAX_NODES = maxN;
        ...
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; j++) {
                if (A[i][j]) {
                    ...
                    if (i > 0 && A[i - 1][j]) {
                        AddToAllEdges(i * cols + j, (i - 1) * cols + j, cost1);
                    }
                    if (i < rows - 1 && A[i + 1][j]) {
                        AddToAllEdges(i * cols + j, (i + 1) * cols + j, cost1);
                    }
                    if (j > 0 && A[i][j - 1]) {
                        AddToAllEdges(i * cols + j, i * cols + j - 1, cost2);
                    }
                    if (j < cols - 1 && A[i][j + 1]) {
                        AddToAllEdges(i * cols + j, i * cols + j + 1, cost2);
                    }
                    ...
                    if (!connect4) {
                        ...
                        if (i < rows - 1 && j < cols - 1 && A[i + 1][j + 1]) {
                            AddToAllEdges(i * cols + j, (i + 1) * cols + j + 1, 1);
                        }
                        if (i > rows - 1 && j > 0 && A[i + 1][j - 1]) {
                            AddToAllEdges(i * cols + j, (i + 1) * cols + j - 1, 1);
                        }
                        if (i > 0 && j < cols - 1 && A[i - 1][j + 1]) {
                            AddToAllEdges(i * cols + j, (i - 1) * cols + j + 1, 1);
                        }
                    }
                }
    ...
    private void AddToAllEdges(int from, int to, int cost) {
        ...
        if (nodes[from] == null) {
            ...
            nodes[from].add(Integer.valueOf(from));
        }

        if (nodes[to] == null) {
            // Create set of connect components [singleton] for this node
            nodes[to] = new HashSet(2 * MAX_NODES);
            nodes[to].add(Integer.valueOf(to));
        }
    }
    ...
    public Vector getAllNewEdges() {
        return allNewEdges;
    }

    public TreeSet getAllEdges() {
        return allEdges;
    }
